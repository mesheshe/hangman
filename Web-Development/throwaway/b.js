...
function bindButton(){
    ...
    document.addEventListener("submit", function(event){ 
        ...
        if (zipInput.value == "" && stateInput.value != ""){
            req.open("GET",`http://api.openweathermap.org/data/2.5/weather?q=${cityInput.value},${stateInput.value},us&units=metric,&appid=${apiKey}`,false);
        }else if (zipInput.value != ""){
            req.open("GET",`http://api.openweathermap.org/data/2.5/weather?q=${zipInput.value},us&units=metric&appid=${apiKey}`,false);
        }else{
            req.open("GET",`http://api.openweathermap.org/data/2.5/weather?q=Error404&appid=${apiKey}`,false);
